...
export const useThemeToggle = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    // Alternate origin: top-right when activating dark mode, bottom-left when activating light mode
    const activeStart: AnimationStart = isDark ? 'bottom-left' : 'top-right';

    const animation = createAnimation(variant, activeStart, blur, gifUrl);
...
export const ThemeToggleButton = ({
  ...
  return (
    <button
      type="button"
      className={cn(
        'flex items-center justify-center cursor-pointer border-0 bg-transparent p-1.5 text-foreground/80 outline-none transition-all duration-300 hover:text-foreground hover:scale-110 active:scale-95',
        className,
      )}
      onClick={toggleTheme}
    ...
    >
      ...
      {isDark ? (
        <Moon className="size-4 sm:size-5 transition-transform" />
      ) : (
        <Sun className="size-4 sm:size-5 transition-transform" />
      )}
    </button>
  );
};
